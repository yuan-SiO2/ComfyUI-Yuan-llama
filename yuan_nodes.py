...
from .skill_core import (
    发现skills as _发现skills,
    获取skill as _获取skill,
    读取skill正文 as _读取skill正文,
    读取reference as _读取skillreference,
)
...
# 官方 MiniMax H3 Skill 包（official_skills/h3-prompt-writing，来自 MiniMax-AI/MiniMax-H3）
...
官方skill不注入 = "不注入（仅核心规则）"


def _官方skill选项() -> list[str]:
    """动态发现 official_skills/ 下的全部 Skill（目录名=选项值），固定附「不注入」入口。"""
    return [官方skill不注入] + [skill["id"] for skill in _发现skills()]


def _官方skill默认值() -> str:
    """默认沿用官方 H3 Skill；目录缺失时退化为「不注入」。"""
    if any(skill["id"] == "h3-prompt-writing" for skill in _发现skills()):
        ...
        return {
            "required": {
                "服务商": (API_MODES, {
                    "default": 本地GGUF模式,
                    "display_name": "服务商",
                    "tooltip": "模式切换：前三种走云端 OpenAI 兼容接口；本地API模型 用 llama.cpp 离线加载。切换后无关参数自动隐藏。",
                }),
                "API密钥": ("STRING", {
                    "default": "",
                    "display_name": "API密钥",
                ...
                "上下文长度": ("INT", {
                    ...
                    "display_name": "上下文长度",
                    "tooltip": "仅本地API模型模式使用。对应 llama.cpp 的 n_ctx；默认 32768；Qwen3.8 原生上限为 262144。",
                }),
                "最大输出长度": ("INT", {
                    "default": _默认最大输出长度, "min": 256, "max": 8192, "step": 256,
                    "display_name": "最大输出长度",
                    "tooltip": "仅本地API模型模式使用。对应 llama.cpp 的 n_predict，默认 4096。",
                }),
                "GPU层数": ("INT", {
...
def _选择skillguide(skill: dict, 生成类型) -> str | None:
    """按生成类型选 guide reference：Ref2VA 优先 ref 文件，其余优先 base/guide 文件，最后取第一个。"""
    refs = skill.get("references") or []
    if not refs:
        return None
    is_ref2va = str(生成类型 or "").startswith("Ref2VA")
    for ref in refs:
        base = os.path.basename(ref).lower()
        if is_ref2va and base in ("ref-en.txt", "ref.txt", "reference.txt"):
            return ref
        if not is_ref2va and base in ("base-en.txt", "base.txt", "guide.txt"):
            return ref
    for ref in refs:
        base = os.path.basename(ref).lower()
        if is_ref2va and base.endswith(".txt") and "ref" in base:
            return ref
        if not is_ref2va and base.endswith(".txt") and ("base" in base or "guide" in base):
            return ref
    return refs[0]


@lru_cache(maxsize=8)
def _官方skill指令(skill_id: str, 生成类型) -> str:
    """按选中的 Skill 加载 SKILL.md（+ 按生成类型选 guide reference），失败或选择「不注入」时返回空串（优雅降级）。"""
    if not skill_id or str(skill_id) == 官方skill不注入:
        return ""
    skill = _获取skill(str(skill_id))
    if skill is None:
        return ""
    try:
        skill_text = _读取skill正文(skill).strip()
    except OSError:
        return ""
    guide_relpath = _选择skillguide(skill, 生成类型)
    guide_text = ""
    if guide_relpath:
        try:
            guide_text = _读取skillreference(skill, guide_relpath).strip()
        except (OSError, ValueError):
            guide_text = ""
    if str(skill_id) == "h3-prompt-writing":
        header = (
        ...
        header = (
            f"VERBATIM_SKILL_SOURCE skill={skill_id}. Apply this Skill's guidance as additional writing "
            "reference. The official H3 output contract, field names, timing, labels, and reference-role rules "
            "still take priority."
        )
    body = f"--- SKILL.md ---\n{skill_text}"
    if guide_text:
...
class 提示词增强器:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "官方Skill": (_官方skill选项(), {"default": _官方skill默认值(), "display_name": "官方 Skill", "tooltip": "注入随插件分发的官方 Skill（SKILL.md + 按生成类型选 guide）。下拉选项自动发现 official_skills/ 目录；选「不注入（仅核心规则）」时跳过 Skill 注入。"}),
    ...
    def run(
        ...
        官方skill = _官方skill指令(官方Skill, 生成类型)
        if 官方skill:
        ...
        # 创意预设属于「官方增强」模式的内容，其他提示词模式下不注入
        创意风格 = 创意预设风格指令.get(str(创意预设 or ""), "")
        if 创意风格 and str(提示词模式 or "") == "官方增强":
            写作指令 = f"创意预设：{创意预设}\n{创意风格}\n\n{写作指令}"
